//ATM SERVICE MANAGMENT PROJECT BY K.M.H.Mubin
...
void welcome()
{
    ...
    gotoxy(33,11); printf("K.M.H.MUBIN");
    gotoxy(24,20);
...
void add_ac()
{
    ...
    if (err!=0) {printf("\a"); cls(10,19,70,19);}
     ...
     do
     {
        gotoxy(10,19); printf("Opening Balance (in TAKA.)      :   ");
        scanf("%f",&ac.c_bal);
...
void deposit()
{
    ...
    gotoxy(5,12); printf("Amount to be Deposited (in TAKA.) : "); scanf("%f",&amt);
    title();
...
void withdraw()//exactly similar to deposit. only difference is amount is subtracted.
{
    ...
    gotoxy(50,10); printf("[ %s ]",nam);
    gotoxy(5,12); printf("Amount to be Withdrawn (in TAKA.)  : "); scanf("%f",&amt);
    fp=fopen("ACCOUNT.TXT","r");
...
void transaction()
{
    ...
    else
    {
        ...
        gotoxy(50,6);puts("   Dr. (TAKA.)");
        gotoxy(65,6);puts("   Cr. (TAKA.)");
...
do
    ...
    int ch,i;
    char ex,key;
    gotoxy(30,6);printf("1. Add Customer");
    gotoxy(30,8);printf("2. Add Customer Information");
    gotoxy(30,10);printf("3. Edit Customer name / Password");
    gotoxy(30,12);printf("4. Delete Customer");
